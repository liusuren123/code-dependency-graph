...
class MultiLayerCodeParser:
    ...
    def parse_repository(
    ...
    ) -> Tuple[List[Symbol], List[Dependency], List[Dict]]:
        """
        解析整个仓库
        ...
        if progress_callback:
            progress_callback('scanning', 1, 1, f'发现 {total_files} 个源文件')
        ...
        for idx, file_path in enumerate(file_list):
            ...
            if progress_callback and (idx + 1) % 10 == 0:
                progress_callback('parsing', idx + 1, total_files, f'解析 {os.path.basename(file_path)}')

        if progress_callback:
            progress_callback('completed', total_files, total_files, f'解析完成: {len(all_symbols)} 符号')

        logger.info(f"递归解析完成: 路径={repo.path}, 扫描文件={total_files}, 符号={len(all_symbols)}, 依赖={len(all_dependencies)}, 数据流={len(all_data_flows)}, 错误路径={len(all_error_paths)}")
